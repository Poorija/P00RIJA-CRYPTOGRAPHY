...
if (!MONITOR_PASSWORD || MONITOR_PASSWORD.length < 6) {
  console.warn('WARNING: MONITOR_PASSWORD is too short or not set. Using insecure fallback "admin123456" for emergency access.');
...
function getMonitorAuth(req) {
  const candidates = [];
  if (req.headers.authorization) candidates.push(req.headers.authorization);
  if (req.query && req.query.auth) candidates.push(`Basic ${req.query.auth}`);
  const cookie = readCookie(req, 'monitor_token_v2');
  if (cookie) candidates.push(`Basic ${cookie}`);
  return candidates.find((candidate) => {
    const decoded = decodeMonitorAuth(candidate);
    return decoded && decoded.user === 'admin' && decoded.pass === MONITOR_PASSWORD;
  }) || candidates[0] || '';
}
...
app.post('/admin/login', (req, res) => {
  ...
  if (password === MONITOR_PASSWORD) {
    ...
    return res.json({ ok: true, token, redirect: '/Monitor_Server' });
  }
...
<body class="p-4 md:p-8">
    ...
    <script>
        ...
        function monitorAuthHeaders(includeJson = false) {
            ...
            const token = localStorage.getItem('monitor_token_v2');
...
function findOpenPresenceByFingerprint(fingerprint = '') {
  ...
  return Array.from(presence.values()).find((client) =>
