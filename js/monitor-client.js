...
/* ============================================================================
   monitor-client.js — the desktop monitor.
   ============================================================================

   Talks to a relay's /healthz and /admin/* over plain fetch. The relay reflects
   the request Origin in Access-Control-Allow-Origin, so cross-origin works from
   the packaged webview without anything special here.

   WHY connect-src IN tauri.monitor.conf.json ALLOWS PLAIN http:

   It used to read `connect-src 'self' ipc: http://ipc.localhost https: wss:`,
   and that quietly broke the monitor's main job. The relay this suite is built
   around is very often a laptop on a LAN reachable at http://192.168.1.34:9000
   — normalizeServerUrl below deliberately picks http for a local address, and
   the app's own relay discovery does the same. Under the old policy every one
   of those requests died as `connect-src -> http://192.168.1.34:9000/healthz`
   with no visible error; only an https relay could ever be monitored.

   The main application keeps a tight connect-src because it holds the user's
   vault, and an injected script there could exfiltrate it. This is a different
   program with a different exposure: it stores one admin password, and its
   entire purpose is connecting to whatever server address the operator types.
   A policy that forbids the addresses operators actually have is not buying
   security, it is just not working.
   ============================================================================ */
...
(function () {
  ...
  const els = {
    ...
    metricCpu: $('metricCpu'),
    metricDiscarded: $('metricDiscarded'),
    metricQuota: $('metricQuota'),
    metricCert: $('metricCert'),
    relayInstance: $('relayInstance'),
    relayLimits: $('relayLimits'),
    relayMailboxes: $('relayMailboxes'),
    relayThrottle: $('relayThrottle'),
    relayDiscardSummary: $('relayDiscardSummary'),
    relayDiscardTable: $('relayDiscardTable'),
  ...
  async function api(path, options = {}) {
    ...
    /* Bearer for a session handle from a relay that issues them, Basic for one
       that does not. Hard-coding Basic here was half of why nothing worked. */
    if (state.token) headers.set('Authorization', `${state.tokenType || 'Basic'} ${state.token}`);
  ...
  function render(data) {
    ...
    renderRelay(data.relay, data.cert);
    renderPeers(peers);
  ...
  /* The relay discards messages on purpose — media ages out, a mailbox over
     quota loses its oldest, a text tail past the limit is trimmed — and until
     now none of that reached a screen. A message that never arrived looked
     exactly like one that was never sent. This panel is that difference. */
  function renderRelay(relay, cert) {
    if (!relay) {
      /* An older relay has no `relay` block. Saying so beats rendering zeros
         that look like a healthy server. */
      if (els.relayInstance) els.relayInstance.textContent = 'این نسخهٔ سرور این گزارش را نمی‌دهد';
      [els.relayLimits, els.relayMailboxes, els.relayThrottle].forEach((list) => {
        if (list) list.innerHTML = '';
      });
      if (els.relayDiscardSummary) els.relayDiscardSummary.textContent = '-';
      if (els.relayDiscardTable) els.relayDiscardTable.innerHTML = '';
      return;
    }
    ...
    const item = (label, value) => `<li><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></li>`;

    if (els.relayInstance) els.relayInstance.textContent = relay.instanceId || '-';

    /* Usage next to its ceiling in the same row. A count on its own is not a
       reading — 412 peers is calm on one machine and the edge on another. */
    if (els.relayLimits) {
      els.relayLimits.innerHTML = [
        item('رده', `${limits.tier || '-'} · ${limits.cpuCount || 0} هسته · ${limits.totalMemMb || 0} MB`),
        item('سوکت‌ها', `${sockets.total || 0} از ${limits.wsMaxTotal || 0}`),
        item('سقف هر آی‌پی', String(limits.wsMaxPerIp || 0)),
        item('اتصال‌های ردشده', `${sockets.refused?.total || 0}`
          + ` (${sockets.refused?.perIp || 0} سقف آی‌پی، ${sockets.refused?.capacity || 0} ظرفیت)`),
        item('صف متن', `${boxes.textEnvelopes || 0} از ${limits.textMailboxLimit || 0}`),
        item('سهمیهٔ رسانه', `${formatBytes(boxes.mediaBytes)} از ${formatBytes(limits.mediaQuotaBytes)} (${boxes.quotaPercent || 0}%)`),
        item('نگهداری رسانه', `${Math.round((limits.mediaRetentionMs || 0) / 86400000)} روز`),
        item('اشتراک اعلان', `${relay.push?.endpoints || 0} روی ${relay.push?.subscribers || 0} کاربر`),
        item('خودنابودی', `${relay.selfDestruct?.records || 0} رکورد`),
      ].join('');
    }

    if (els.relayMailboxes) {
      const rows = Array.isArray(boxes.fullest) ? boxes.fullest : [];
      els.relayMailboxes.innerHTML = rows.length
        ? rows.map((row) => item(
            `${row.fingerprint}… (${row.envelopes})`,
            `${formatBytes(row.mediaBytes)} · ${row.quotaPercent}%`)).join('')
        : '<li><span>هیچ صندوقی در صف نیست</span><strong>-</strong></li>';
    }

    if (els.relayThrottle) {
      /* Refused addresses sit alongside throttled ones: both are people who
         tried and could not. A refusal is the more urgent of the two, because
         the person on the other end saw nothing but a failure to connect. */
      const turnedAway = (sockets.refused?.addresses || [])
        .map((entry) => item(`ردشده · ${entry.address}`, `${entry.count} بار`));
      const rows = Array.isArray(throttle.nearOrOverLimit) ? throttle.nearOrOverLimit : [];
      const throttled = rows.map((row) => item(
        `${row.bucket} · ${row.client}`,
        `${row.count}/${row.ceiling}${row.over ? ' ⛔' : ''}`));
      els.relayThrottle.innerHTML = turnedAway.concat(throttled).join('')
        || `<li><span>هیچ‌کس نزدیک سقف نیست</span><strong>${throttle.activeBuckets || 0} فعال</strong></li>`;
    }

    if (els.relayDiscardSummary) {
      const reasons = Object.entries(discarded.byReason || {})
        .map(([reason, count]) => `${reason}: ${count}`).join(' · ');
      els.relayDiscardSummary.textContent = discarded.total
        ? `${discarded.last24h || 0} در ۲۴ ساعت، ${discarded.total} روی هم${reasons ? ' — ' + reasons : ''}`
        : 'هیچ پیامی دور ریخته نشده';
    }

    if (els.relayDiscardTable) {
      const rows = Array.isArray(discarded.recent) ? discarded.recent : [];
      els.relayDiscardTable.innerHTML = rows.length
        ? rows.map((row) => `<tr>
            <td>${escapeHtml(row.to || '-')}…</td>
            <td>${escapeHtml(row.from || '-')}…</td>
            <td>${escapeHtml(row.class || '-')}</td>
            <td>${escapeHtml(row.reason || '-')}</td>
            <td>${escapeHtml(formatDate(row.queuedAt))}</td>
            <td>${escapeHtml(formatDate(row.expiredAt))}</td>
          </tr>`).join('')
        : '<tr><td colspan="6">چیزی دور ریخته نشده است.</td></tr>';
    }

    if (els.metricDiscarded) els.metricDiscarded.textContent = String(discarded.last24h || 0);
    if (els.metricQuota) els.metricQuota.textContent = `${boxes.quotaPercent || 0}%`;
    if (els.metricCert) {
      els.metricCert.textContent = cert && Number.isFinite(cert.daysLeft)
        ? `${cert.daysLeft} روز`
        : '-';
    }
  }
  ...
  async function login(event) {
    ...
    try {
      ...
      /* `bearer: true` asks for the session handle in the response body. This
         client has no cookie jar shared with the relay — it is a different
         origin and the fetch above sends no credentials — so without asking it
         received nothing, wrote `undefined` into state.token, sent no
         Authorization header at all, and every subsequent call came back
         "Invalid credentials". That read as a wrong password and was not. */
      const payload = await api('/admin/login', {
        method: 'POST',
        body: JSON.stringify({ password, bearer: true }),
      });
      if (payload.token) {
        state.token = payload.token;
        state.tokenType = payload.tokenType || 'Bearer';
      } else {
        /* An older relay does not return one. It still accepts HTTP Basic, and
           this app ships separately from the server, so it should keep working
           against one that has not been updated yet. */
        state.token = base64Utf8(`admin:${password}`);
        state.tokenType = 'Basic';
      }
      localStorage.setItem('poorija-monitor-server-url', state.serverUrl);
      localStorage.setItem('poorija-monitor-token', state.token);
      localStorage.setItem('poorija-monitor-token-type', state.tokenType);
  ...
  async function logout() {
    ...
    localStorage.removeItem('poorija-monitor-token-type');
    window.clearInterval(state.refreshTimer);
  ...
  async function changePassword(event) {
    ...
    try {
      ...
      /* change-password revokes every session on the relay, so the bearer
         handle just died. Basic with the new password keeps this window
         working; the scheme has to move with the token or the next call is
         a Bearer header carrying a Basic credential. */
  ...
  function restoreSession() {
    ...
    state.serverUrl = serverUrl;
    state.token = token;
    /* Restored with its scheme. A Basic credential replayed as Bearer is
       rejected, and the failure reads as an expired session. */
    state.tokenType = localStorage.getItem('poorija-monitor-token-type') || 'Basic';
